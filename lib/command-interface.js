 ...
 *      optionThatIWantCommandToHave: value
 *    });
 *
 *    // invoked with line is something like: "dothing x <enter>"
 *    ci.commandFactory('dothing', {
 *      // handles line feeds
 *      action: function(argv, callback) {
 *        // callback must be invoked for prompt to display again
 *      },
 *
 *      // handles tab completion
 *      completer: function(argv, callback) {
 *        // callback must be invoked with readline expected values:
 *        callback(null, [['myresults', ...], 'completedword']);
 *      },
 *
 *      help: function() {
 *        // help message
 *        return 'this thing does X';
 *      }
 *    });
 *
 *    ci.start(
 *      'my readline terminal>', // optional
 *      true // shows help message
 *    );
 */
